    ...
    for template in load_template_files():
        model = db.scalar(
            select(DocumentTemplate).where(
                DocumentTemplate.template_key == template.key
            )
        )
        if not model:
        ...
        existing = {
            ...
            for field in db.scalars(
                select(FieldDefinition).where(FieldDefinition.template_id == model.id)
            ).all()
        }
        for field in template.fields:
            ...
            if not row:
                row = FieldDefinition(
                    template_id=model.id, field_key=field.key, display_label=field.label
                )
                db.add(row)
...
def infer_template_key(
    text: str, parser_type: str, requested_key: str | None = None
) -> str | None:
    ...
    for template in load_template_files():
        ...
        for field in template.fields:
            score += sum(
                1
                for alias in [field.label, field.key, *(field.aliases or [])]
                if alias and alias.lower().replace("_", " ") in lower
            )
        if score > best_score:
            best_score = score
...
def match_template_field(
    template: TemplateDefinition, field_name: str
) -> TemplateField | None:
    ...
    for field in template.fields:
        names = {
            _normalize_key(field.key),
            _normalize_key(field.label),
            *{_normalize_key(alias) for alias in (field.aliases or [])},
        }
        if candidates & names:
            return field
    return None
...
def apply_template_to_fields(
    fields, template_key: str | None, field_factory: Callable[..., object] | None = None
):
    ...
    for field in fields:
        ...
        if match:
            ...
            if existing is None or float(getattr(field, "confidence", 0.0)) > float(
    ...
    missing_required = [
        field
        for field in template.fields
        if field.required and field.key not in matched_required_keys
    ]
    ...
    def _template_sort_key(item) -> tuple[int, int, int, str]:
        ...
        standard_key = str(
            getattr(item, "standard_field_key", "") or name.rsplit(".", 1)[-1]
        )
        is_record_row = 1 if name.startswith("records[") else 0
        return (
            is_record_row,
            _record_sort_index(name),
            order_by_key.get(standard_key, 9999),
            name,
        )

    ordered_template_fields = sorted(
        [*official_by_identity.values(), *missing_rows], key=_template_sort_key
    )
...
def template_prompt_contract(template_key: str | None) -> str:
    ...
    for field in template.fields:
        ...
        aliases = (
            f" Aliases: {', '.join(field.aliases or [])}." if field.aliases else ""
        )
        validation = f" Validation: {field.validation}." if field.validation else ""
        lines.append(
            f"- {field.key} ({field.data_type}, {required}): {field.label}.{aliases}{validation}"
        )
    lines.append(
        "Do not invent new official field names. If extra useful data appears, label it clearly as a suggested extra field."
    )
    return "\n".join(lines)
