...
def validate_value(
    ...
    if required and _is_empty(value):
        return ValidationResult(
            "missing_required",
            f"{label} is required by the selected template but no value was found.",
            "required",
            True,
        )
    if _is_empty(value):
        return ValidationResult(
            "not_applicable", "Optional field is empty.", "optional_empty", False
        )
    ...
    if data_type == "email":
        ...
        return ValidationResult(
            ...
            "Valid email address." if ok else "Value is not a valid email address.",
            "email",
            not ok,
        )
    ...
    if data_type == "date":
        ...
        return ValidationResult(
            "valid" if ok else "invalid",
            "Valid date format." if ok else "Value is not a supported date format.",
            "date",
            not ok,
        )
    ...
    if data_type == "currency":
        ...
        allowed_currencies = validation.get("currencies") or [
            "RM",
            "MYR",
            "USD",
            "EUR",
        ...
        has_currency = any(
            str(code).lower() in text.lower() for code in allowed_currencies
        )
        min_value = validation.get("min")
        ok = amount is not None and (
            has_currency or not validation.get("require_currency_symbol", False)
        )
        if min_value is not None and amount is not None:
            ok = ok and amount >= float(min_value)
        if ok:
            return ValidationResult(
                "valid", "Valid currency/amount value.", "currency", False
            )
        return ValidationResult(
            "invalid",
            "Value is not a valid currency/amount for this template field.",
            "currency",
            True,
        )
    ...
    if data_type == "currency_code":
        ...
        return ValidationResult(
            ...
            "Valid currency code."
            if ok
            else f"Currency code must be one of: {', '.join(map(str, allowed))}.",
            "currency_code",
            not ok,
        )
    ...
    if data_type == "percentage":
        ...
        return ValidationResult(
            ...
            if ok
            else "Percentage must be a number between 0 and 100.",
            "percentage",
            not ok,
        )
    ...
    if data_type in {"number", "integer"}:
        ...
        return ValidationResult(
            "valid" if ok else "invalid",
            "Valid numeric value." if ok else "Value must be numeric.",
            data_type,
            not ok,
        )
    ...
    if pattern and not re.search(str(pattern), text):
        return ValidationResult(
            "invalid", "Value does not match the configured pattern.", "regex", True
        )
