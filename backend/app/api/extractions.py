...
from ..services.review_service import mark_extraction_accepted, refresh_document_review_status
...
def add_missing_extraction(document_id: int, payload: AddExtractionIn, db: Session = Depends(get_db)):
    ...
    if document.status == DocumentStatus.REJECTED.value:
        raise HTTPException(status_code=409, detail="Rejected documents are view-only and cannot be modified.")
    ...
    if document.document_type_key:
        ...
        if match:
            ...
            stored_field_name = f"suggested_extra_fields.{field_name.lower().replace(' ', '_')}"
            data_type = "suggestion"

    validation_result = validate_value(stored_field_name, value, data_type, required, validation)
...
def correct_extraction(extraction_id: int, payload: CorrectionIn, db: Session = Depends(get_db)):
    ...
    new_field_name = extraction.field_name if template_field_locked else requested_field_name
    ...
    if template_field_locked and requested_field_name != extraction.field_name:
        raise HTTPException(status_code=409, detail="Template field names are locked. Edit the value only so standardized client schemas remain consistent.")
...
def accept_extraction(extraction_id: int, payload: AcceptExtractionIn, db: Session = Depends(get_db)):
    ...
    if validation_result.requires_review:
        extraction.accepted = False
        extraction.needs_review = True
        db.add(AuditLog(document_id=extraction.document_id, action="field_acceptance_blocked_by_validation", actor=payload.accepted_by, details=f'{extraction.field_name}: cannot be accepted because validation failed: {validation_result.message}'))
    else:
...
def delete_extraction(extraction_id: int, actor: str = "demo_user", db: Session = Depends(get_db)):
    extraction = db.scalar(select(Extraction).where(Extraction.id == extraction_id).options(selectinload(Extraction.corrections)))
    if not extraction:
        raise HTTPException(status_code=404, detail="Extraction not found")
    ...
    if not removable:
        raise HTTPException(status_code=409, detail="Only suggested extra fields or manually added fields can be removed.")
