...
def parse_upload(filename: str | None, content_type: str | None, content: bytes) -> ParsedDocument:
    ...
    if suffix in {'.txt', '.md'} or ctype in {'text/plain', 'text/markdown'}:
        ...
        return ParsedDocument(raw_text=text, parser_type='plain_text', normalized_preview=text[:500])
...
def parse_raw_text(raw_text: str) -> ParsedDocument:
    return ParsedDocument(raw_text=raw_text, parser_type='raw_text', normalized_preview=raw_text[:500])
...
def _parse_csv(content: bytes) -> ParsedDocument:
    ...
    normalized = '\n'.join(lines)
    return ParsedDocument(raw_text=normalized, parser_type='csv', normalized_preview=normalized[:500])
...
def _parse_json(content: bytes) -> ParsedDocument:
    ...
    if isinstance(parsed, list):
        ...
        prefix = 'JSON document containing an object with keys: ' + ', '.join(map(str, parsed.keys())) + '.\n'
    else:
        prefix = f'JSON document containing {type(parsed).__name__}.\n'
    raw_text = prefix + pretty
    return ParsedDocument(raw_text=raw_text, parser_type='json', normalized_preview=raw_text[:500])
