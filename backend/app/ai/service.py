...
class AISettings:
    ...
    def from_env(cls) -> "AISettings":
        ...
        return cls(
            ...
            defer_on_rate_limit=os.getenv("AI_DEFER_ON_RATE_LIMIT", "true").strip().lower() in {"1", "true", "yes", "on"},
            review_assistant_enabled=os.getenv("AI_REVIEW_ASSISTANT_ENABLED", "true").strip().lower() in {"1", "true", "yes", "on"},
            review_assistant_required=os.getenv("AI_REVIEW_ASSISTANT_REQUIRED", "false").strip().lower() in {"1", "true", "yes", "on"},
        )
