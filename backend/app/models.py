...
class Document(Base):
    ...
    def original_preview_text(self) -> str:
        """Best-effort text preview from the immutable uploaded bytes.

        The original bytes are the source of truth for download/audit. For text,
        JSON, CSV, and Markdown, this returns the exact uploaded text. For PDFs,
        it returns the extracted selectable text because browser-side PDF binary
...
class Extraction(Base):
    ...
    # Optional source-location metadata used by the review workspace for
    # deterministic highlighting. Snippet matching remains a fallback for
    # older rows and AI-generated snippets.
...
class DocumentAIReview(Base):
    """AI-generated reviewer briefing for a processed document.

    This is intentionally separate from field extraction rows. The assistant
    summarizes the document, highlights potential issues, and gives a review
    recommendation, but it never approves/rejects documents or changes field
    values.
    """
...
class AIRequestCache(Base):
    """Cached normalized AI extraction results keyed by document content/provider/model.

    This prevents repeated AI calls when the same content is uploaded or a job is
    requeued after a transient failure. Cached data is still copied into normal
    extraction rows for each document so the review/audit workflow stays intact.
    """
...
class AIRateLimitWindow(Base):
    ...
    PostgreSQL tracks request windows so multiple Celery workers coordinate and
    avoid stampeding the provider when minute/day limits are reached.
    """
