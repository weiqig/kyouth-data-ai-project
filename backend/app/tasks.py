...
from .services.template_service import (
    apply_template_to_fields,
    infer_template_key,
    get_template_definition,
    template_prompt_contract,
)
from .services.template_extraction_service import (
    extract_template_labelled_fields,
    required_field_keys,
)
...
def _classify(text: str, parser_type: str) -> tuple[str, float, str]:
    ...
    if parser_type == "csv":
        return (
            "structured_csv",
            0.92,
            "CSV parser normalized rows and columns before extraction.",
        )
    if parser_type == "json":
        return (
            "structured_json",
            0.92,
            "JSON parser validated and pretty-printed the object before extraction.",
        )
    ...
    if any(
        token in lower
        for token in ["contract", "agreement", "effective date", "termination"]
    ):
        return (
            "contract_or_agreement",
            0.78,
            "Agreement-related keywords were detected.",
        )
...
def _extract_structured_csv(text: str) -> list[ExtractedField] | None:
    ...
    summary = f"{len(records)} CSV record(s)" + (
    ...
    fields: list[ExtractedField] = [
        ExtractedField(
            ...
            1.0,
            text[:220],
            "CSV parsed successfully into structured rows.",
            True,
        ),
        ExtractedField(
            "summary",
            ...
            1.0,
            summary,
            "Summary derived from parsed CSV row and column counts.",
            True,
        ),
        ExtractedField(
            "record_count",
            str(len(records)),
            1.0,
            f"CSV row count: {len(records)}",
            "Counted parsed CSV records.",
            True,
        ),
        ExtractedField(
            "schema_fields",
        ...
        ExtractedField(
            ...
            0.95,
            text[:160],
            "CSV rows represent a collection of structured records.",
            True,
        ),
    ]
        ...
        for key, value in row.items():
            ...
            fields.append(
                ExtractedField(
                    ...
                    reasoning="Directly copied from parsed CSV; no AI inference required."
                    if not needs_manual_check
                    else "CSV field was present but empty, so it remains reviewable.",
...
def _extract_structured_json(text: str) -> list[ExtractedField] | None:
    ...
    if isinstance(parsed, list):
        ...
        summary = f"{record_count} JSON record(s)" + (
        ...
        fields.append(
            ExtractedField(
                ...
                1.0,
                text[:220],
                "Valid JSON array parsed successfully.",
                True,
            )
        )
        fields.append(
            ExtractedField(
                ...
                1.0,
                summary,
                "Summary derived from JSON array length and object keys.",
                True,
            )
        )
        fields.append(
            ExtractedField(
                ...
                str(record_count),
                1.0,
                f"Array length: {record_count}",
                "Counted top-level JSON array items.",
                True,
            )
        )
        if schema_keys:
            fields.append(
                ExtractedField(
                    ...
                    1.0,
                    ", ".join(schema_keys),
                    "Collected unique keys from JSON object records.",
                    True,
                )
            )
        fields.append(
            ExtractedField(
                ...
                0.95,
                text[:160],
                "Top-level JSON array indicates a collection of structured records.",
                True,
            )
        )
            ...
            if isinstance(item, dict):
                for key, value in item.items():
                    for suffix, flattened_value in _flatten_json(value):
                        field_name = (
                            f"records[{idx}].{key}"
                            if suffix == str(key) or not suffix
                            else f"records[{idx}].{key}.{suffix}"
                        )
                        ...
                        fields.append(
                            ExtractedField(
                                ...
                                confidence=1.0,
                                snippet=_json_source_snippet(
                                    field_name, flattened_value
                                ),
                ...
                fields.append(
                    ExtractedField(
                        ...
                        confidence=1.0,
                        snippet=_json_source_snippet(
                            f"records[{idx}]", json.dumps(item, ensure_ascii=False)
                        ),
    ...
    if isinstance(parsed, dict):
        ...
        fields.append(
            ExtractedField(
                "summary",
                f"JSON object with fields: {', '.join(keys)}.",
                1.0,
                ", ".join(keys),
                "Summary derived from JSON object keys.",
                True,
            )
        )
        fields.append(
            ExtractedField(
                ...
                1.0,
                ", ".join(keys),
                "Collected keys from the top-level JSON object.",
                True,
            )
        )
    ...
    fields.append(
        ExtractedField(
            "document_type",
            f"structured_json_{type(parsed).__name__}",
            1.0,
            text[:220],
            "Valid scalar JSON parsed successfully.",
            True,
        )
    )
    fields.append(
        ExtractedField(
            ...
            1.0,
            text[:220],
            "Directly copied JSON scalar value.",
            True,
        )
    )
    return fields
...
def deterministic_extract(text: str, parser_type: str) -> list[ExtractedField]:
    ...
    fields.append(
        ExtractedField(
            "document_type", doc_type, doc_confidence, text[:160], doc_reason
        )
    )
    ...
    fields.append(
        ExtractedField(
            "summary",
            summary,
            0.70 if summary else 0.40,
            summary[:220],
            "Brief extractive summary generated from the first meaningful lines.",
        )
    )
    ...
    if email:
        ...
        fields.append(
            ExtractedField(
                ...
                0.92,
                _snippet(text, value),
                "Regex matched a valid email-like token.",
            )
        )

    amount = re.search(
        r"(?:RM|MYR|USD|EUR|GBP|\$)\s?\d{1,3}(?:,\d{3})*(?:\.\d{2})?", text, re.I
    )
    if amount:
        ...
        fields.append(
            ExtractedField(
                ...
                0.84,
                _snippet(text, value),
                "Currency pattern detected in the source text.",
            )
        )
    ...
    phone = re.search(
        r"\b(?:\+?\d{1,3}[-.\s]?)?(?:\(?\d{2,4}\)?[-.\s]?)?\d{3,4}[-.\s]?\d{4}\b", text
    )
    if phone:
        value = phone.group(0)
        fields.append(
            ExtractedField(
                "phone_or_reference_number",
                value,
                0.66,
                _snippet(text, value),
                "Number pattern detected; may be a phone, account, or reference number.",
            )
        )
    ...
    if len(fields) <= 2:
        fields.append(
            ExtractedField(
                ...
                "No high-confidence business fields found.",
                0.48,
                text[:220],
                "The document can still be reviewed and corrected by a human.",
            )
        )
...
def extract_for_document(
    text: str, parser_type: str, db=None, template_key: str | None = None
) -> tuple[list[ExtractedField], str, str | None]:
    ...
    if template_labelled_fields and labelled_template:
        found_required = {
            item.standard_field_key
            for item in template_labelled_fields
            if item.required and item.value.strip()
        }
        ...
        if required_keys and required_keys.issubset(found_required):
            fields, template, missing = apply_template_to_fields(
                template_labelled_fields, template_key, field_factory=ExtractedField
            )
            suffix = f":template:{template.key}" if template else ""
            return (
                fields,
                f"deterministic_labelled{suffix}",
    ...
    if should_use_ai(parser_type, settings):
        ...
        effective_parser_type = (
            f"{parser_type}:template:{template_key}" if template_key else parser_type
        )
        ...
        if db is not None:
            ...
            if cached and cached.fields:
                fields = [
                    *template_labelled_fields,
                    *_ai_fields_to_extracted_fields(cached),
                ]
                fields, template, missing = apply_template_to_fields(
                    fields, template_key, field_factory=ExtractedField
                )
                suffix = f":template:{template.key}" if template else ""
                return (
                    fields,
                    f"ai_cache:{cached.provider}:{cached.model}{suffix}",
                    template.key if template else None,
                )
        ...
        try:
            if db is not None:
                reserve_ai_request_quota(
                    db, provider=provider.provider_name, model=provider.model
                )
                # Persist quota reservation before making the external request so
                # parallel workers share one PostgreSQL-backed counter.
                db.commit()

            ai_result = provider.extract_fields(
                document_text=ai_document_text, parser_type=effective_parser_type
            )
            ...
            fields = [
                *template_labelled_fields,
                *_ai_fields_to_extracted_fields(ai_result),
            ]
            if fields:
                fields, template, missing = apply_template_to_fields(
                    fields, template_key, field_factory=ExtractedField
                ...
                return (
                    fields,
                    f"ai:{ai_result.provider}:{ai_result.model}{suffix}",
                    template.key if template else None,
                )
            if not settings.fallback_to_deterministic:
                return (
                    [],
                    f"ai:{ai_result.provider}:{ai_result.model}:empty",
                    template_key,
                )
            ...
            fallback_fields = [
                *template_labelled_fields,
                *deterministic_extract(text, parser_type),
            ]
            ...
            fallback_fields, template, missing = apply_template_to_fields(
                fallback_fields, template_key, field_factory=ExtractedField
            )
            return (
                fallback_fields,
                f"deterministic_fallback_after_ai_rate_limit:{settings.provider}",
                template.key if template else None,
            )
            ...
            return (
                fallback_fields,
                f"deterministic_fallback_after_ai_error:{settings.provider}",
                template.key if template else None,
            )
...
def _build_ai_review_context(
    doc: Document, fields: list[ExtractedField], extraction_strategy: str
) -> str:
...
def _create_ai_review_if_enabled(
    db,
    doc: Document,
    fields: list[ExtractedField],
    ...
    db.add(
        DocumentAIReview(
            ...
            consistency_checks_json=json.dumps(
                result.consistency_checks, ensure_ascii=False
            ),
        )
    )
    db.add(
...
def process_document_job(self, job_id: int) -> None:
    ...
    try:
        ...
        if doc.status == DocumentStatus.REJECTED.value:
            job.status = JobStatus.FAILED.value
            job.error_message = (
                "Document was rejected before processing; worker skipped it."
            )
        ...
        db.add(
            AuditLog(
                ...
                actor="worker",
                details=f"Job {job.id} started from PostgreSQL state.",
            )
        )
        db.commit()
        ...
        try:
            ...
            if doc.extraction_mode == "discovery":
                ...
                selected_template_key = infer_template_key(
                    doc.raw_text, doc.parser_type
                )

            extracted_fields, extraction_strategy, applied_template_key = (
            ...
            if applied_template_key:
                ...
                doc.document_type_label = (
                    template.name if template else applied_template_key
                )
        ...
        if doc.status == DocumentStatus.REJECTED.value:
            job.status = JobStatus.FAILED.value
            job.error_message = (
                "Document was rejected while processing; extraction write was skipped."
            )
            job.finished_at = func.now()
            db.add(
                AuditLog(
        ...
        if doc.document_type_key:
            for definition in db.scalars(
                ...
                definition_ids[(doc.document_type_key, definition.field_key)] = (
                    definition.id
                )
        for item in extracted_fields:
            ...
            field_needs_review = (
                ...
                or requires_confidence_review(item.confidence)
                or not item.auto_accept
            )
            ...
            db.add(
                Extraction(
                    document_id=doc.id,
                    field_name=item.field_name,
                    field_definition_id=definition_ids.get(
                        (
                            item.template_key or doc.document_type_key or "",
                            item.standard_field_key or "",
                        )
                    ),
        ...
        # Optional AI Review Assistant: one holistic reviewer briefing after
        # extraction + template enforcement + validation. This does not change
        # field values or approval rules.
        ai_review_created = False
        try:
            ai_review_created = _create_ai_review_if_enabled(
                db, doc, extracted_fields, extraction_strategy, AISettings.from_env()
            )
        except AIRateLimitExceeded as exc:
            settings = AISettings.from_env()
            ...
            db.add(
                AuditLog(
                    ...
                    actor="worker",
                    details=f"AI Review Assistant skipped because provider quota was reached: {exc}",
                )
            )
        except Exception as exc:  # noqa: BLE001
            settings = AISettings.from_env()
            db.rollback()
            # Reattach job/document after rollback before recording non-fatal assistant failure.
            ...
            if doc:
                db.add(
                    AuditLog(
                        ...
                        actor="worker",
                        details=f"AI Review Assistant failed non-fatally: {exc}",
                    )
                )
        ...
        db.add(
            AuditLog(
                document_id=doc.id,
                action="extraction_strategy_selected",
                actor="worker",
                details=f"Extraction strategy: {extraction_strategy}.",
            )
        )
        db.add(
            AuditLog(
                document_id=doc.id,
                action="structured_fields_auto_accepted",
                actor="worker",
                details=f"Auto-accepted {auto_accepted_count} field(s); {review_required_count} field(s) still require review. Confidence threshold: {round(threshold * 100)}%. Validation rules were applied to extracted/template fields. AI Review Assistant: {'created' if ai_review_created else 'not created'}.",
            )
        )
        db.add(
            AuditLog(
                ...
                actor="worker",
                details=f"Job {job.id} completed and extraction rows committed. Document status is {doc.status}.",
            )
        )
